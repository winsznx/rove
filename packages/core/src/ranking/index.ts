import { RoutePath, PathKind } from '../routes/index.js';
import { toDecimal } from '../decimal/index.js';

const OPERATIONAL_COMPLEXITY: Record<PathKind, number> = {
  convert: 1,
  spot: 2,
  usd_m_perp: 3,
  coin_m_perp: 4,
 ...
 */
export function rankRoutes(routes: RoutePath[]): RoutePath[] {
  ...
  return sorted.map((route) => {
    if ((route.status === 'VALID' || route.status === 'SELECTED') && !foundWinner) {
      foundWinner = true;
      return { ...route, status: 'SELECTED' as const };
    }
    return route;
  });
}
