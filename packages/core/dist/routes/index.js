import { calculateSpotCost, calculateConvertCost, calculateUsdMCost } from '../cost/index.js';
import { evaluateRouteConstraints } from '../constraints/index.js';
 ...
 */
export function generateAndEvaluateRoutes(input) {
    ...
    // Derive trade size
    let requestedBaseQuantity;
    let requestedQuoteNotional;
    if (intent.amount) {
    ...
    if (snapshot.spot) {
        ...
        try {
            ...
            execution = calculateSpotCost(walk, midPrice, feeBps, side, new Date(snapshot.spot.timestamp).toISOString());
        }
        catch {
    ...
    if (snapshot.convert) {
        ...
        const refPrice = snapshot.spot
            ...
            : effectivePrice;
        execution = calculateConvertCost(effectivePrice, refPrice, side, snapshot.convert.quoteId, new Date(snapshot.convert.timestamp).toISOString());
        const disposesUnderlying = side === 'sell';
    ...
    if (snapshot.usdM) {
        ...
        if (isApplicableForObjective) {
            ...
            const costOutput = calculateUsdMCost(snapshot.usdM.markPrice, snapshot.usdM.currentFundingRateBps, snapshot.usdM.fundingIntervalHours, takerFeeBps, intent.horizon, '0.00', new Date(snapshot.usdM.timestamp).toISOString());
